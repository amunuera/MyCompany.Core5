...
namespace MyCompany.Core5.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly Core5DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = Core5Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(Core5DbContext context)
        {
            _context = context;
        }
